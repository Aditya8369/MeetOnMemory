...
export const generateQuizFromTranscript = async (meetingId) => {
  ...
  if (!transcript && !structuredMoM) {
    throw new Error(`No transcript or summary available for meeting: ${meetingId}`)
  }
...
`
  ...
  if (!parsed || !Array.isArray(parsed.questions) || parsed.questions.length === 0) {
    throw new Error('Failed to parse Gemini JSON output for meeting quiz or no questions generated')
  }
