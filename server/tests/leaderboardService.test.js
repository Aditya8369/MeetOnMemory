...
describe("LeaderboardEngine", () => {
  ...
  describe("getFilteredLeaderboard", () => {
    it("should throw error if orgId is missing", async () => {
      await expect(LeaderboardEngine.getFilteredLeaderboard({}))
    ...
    it("should aggregate all-time scores without filters", async () => {
      ...
      const result = await LeaderboardEngine.getFilteredLeaderboard({ orgId: new mongoose.Types.ObjectId().toString(), period: "all" });
    ...
    it("should apply team filter correctly in pipeline", async () => {
      ...
      const matchTeamStage = callArgs.find(stage => stage.$match && stage.$match["userDetails.team"] === "Engineering");
    ...
    it("should apply period filter (week) and reduce points", async () => {
      ...
      const callArgs = GamificationScore.aggregate.mock.calls[0][0];
      const addFieldsStage = callArgs.find(stage => stage.$addFields && stage.$addFields.filteredHistory);
      expect(addFieldsStage).toBeDefined();
    });

    it("should calculate percentiles accurately", async () => {
      const mockAggregatedData = [
        { _id: "score1", user: "user1", userDetails: { _id: "user1" }, calculatedPoints: 100 },
        { _id: "score2", user: "user2", userDetails: { _id: "user2" }, calculatedPoints: 80 },
        { _id: "score3", user: "user3", userDetails: { _id: "user3" }, calculatedPoints: 60 },
        { _id: "score4", user: "user4", userDetails: { _id: "user4" }, calculatedPoints: 40 }
      ];

      GamificationScore.aggregate.mockResolvedValueOnce(mockAggregatedData);
      GamificationScore.aggregate.mockResolvedValueOnce([]);

      const result = await LeaderboardEngine.getFilteredLeaderboard({ orgId: new mongoose.Types.ObjectId().toString() });
