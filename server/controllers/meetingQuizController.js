...
export const getQuizForMeeting = async (req, res) => {
  try {
    ...
    if (!quiz) {
      return res.status(404).json({ message: 'No quiz found for this meeting' })
    }
...
export const getQuizAnalytics = async (req, res) => {
  try {
    ...
    const quiz = await MeetingQuiz.findOne({ meetingId }).lean()
    if (!quiz) {
      return res.status(404).json({ message: 'Quiz not found for this meeting' })
    }
    ...
    responses.forEach((response) => {
      ...
      response.answers.forEach((answer) => {
        ...
        if (question && answer.selectedOptionIndex !== question.correctOptionIndex) {
          missedCounts[answer.questionIndex] = (missedCounts[answer.questionIndex] || 0) + 1
        }
      })
    })
