...
export const resendDigest = async (req, res, next) => {
  try {
    ...
    return sendSuccess(res, { recipientsSentTo: result.recipientsSentTo }, result.message)
  } catch (err) {
    next(err)
  }
}
...
/**
 * Controller to fetch per-meeting digest status and delivery history (#2022).
 */
export const getMeetingDigestStatus = async (req, res, next) => {
  try {
    const { id: meetingId } = req.params

    const result = await MeetingDigestService.getMeetingDigestStatus(meetingId)

    if (!result.success) {
      return res.status(404).json({ success: false, message: result.message })
    }

    return sendSuccess(res, result.data, 'Meeting digest status fetched successfully')
  } catch (err) {
    next(err)
  }
}
