...
const orgIdFromReq = (req) => req.user?.organization?._id || req.user?.organization || null
...
export const getImportanceRecalculationStatus = async (req, res) => {
  try {
    ...
    return sendError(res, 500, 'Failed to load importance recalculation status')
  }
}
...
export const triggerImportanceRecalculation = async (req, res) => {
  try {
    ...
    return sendSuccess(res, result, 'Importance score recalculation triggered successfully', 202)
  } catch (error) {
    if (error.statusCode) {
      return sendError(res, error.statusCode, error.message)
    }
    console.error('Error in triggerImportanceRecalculation:', error)
    return sendError(res, 500, 'Failed to trigger importance score recalculation')
  }
}
