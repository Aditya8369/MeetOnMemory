...
vi.mock('../../utils/mediaStream', () => ({
  ...
  getTrackEnabledState: vi.fn().mockReturnValue({ micOn: true, cameraOn: true }),
}))
...
vi.mock('../../components/meeting-details/AgendaTimer.jsx', () => ({
  ...
  default: ({ meeting, compact }) => (
    <div data-testid={compact ? 'agenda-timer-banner' : 'agenda-timer'}>Agenda</div>
  ),
}))
...
vi.mock('../../components/meetings/TranscriptPanel.jsx', () => ({
  default: ({ showTranscript }) =>
    showTranscript ? <div data-testid="meeting-room-transcript-panel">Transcript</div> : null,
}))
...
const joinMeeting = async () => {
  ...
  await waitFor(() => {
    expect(screen.getByRole('banner', { name: /meeting room header/i })).toBeInTheDocument()
  })
}
...
describe('MeetingRoom — CollaborativeCanvas panel (Issue #2234)', () => {
  ...
  it('does NOT render canvas panel by default after joining', async () => {
    ...
    expect(screen.queryByTestId('meeting-room-canvas-panel')).not.toBeInTheDocument()
  })
