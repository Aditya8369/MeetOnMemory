...
const AdminPanel = () => {
  ...
  const fetchOverviewData = useCallback(async () => {
    ...
    try {
      const [membersRes, orgsRes, meetingsRes, requestsRes, auditRes] = await Promise.allSettled([
      ...
      setOverviewStats({
        ...
        totalMeetings:
          ...
          (meetingsRes.value.data?.meetings || meetingsRes.value.data?.total !== undefined)
            ? (meetingsRes.value.data.meetings?.length ?? meetingsRes.value.data.total ?? 0)
      ...
      <div className="flex flex-1 pt-16">
        ...
        <div className="flex-1 min-w-0 px-4 sm:px-6 lg:px-8 py-6">
            ...
            <div className="mb-6 p-4 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/40 text-amber-800 dark:text-amber-300 text-sm rounded-2xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shadow-xs">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  Rate limiting fallback is active and real-time features may experience lag or
                  cache misses.
                </div>
              </div>
              <a
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-3 mb-1">
                ...
                <h1 className="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">
                  {active.labelKey.includes('.') ? t(active.labelKey) : active.labelKey}
                </h1>
              </div>
          ...
          {activeModule === 'overview' ? (
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Organizations</h3>
                <button
                  type="button"
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400 dark:text-slate-500 flex items-center gap-1 mt-0.5">
                          ...
                          {m.embeddingIndex?.lastIndexedAt
                            ...
                            : ''}
                          {m.embeddingIndex?.status ? ` · ${m.embeddingIndex.status}` : ''}
                        </p>
                        {m.embeddingIndex?.lastError ? (
                          <p className="text-xs text-rose-500 mt-1">{m.embeddingIndex.lastError}</p>
                        ) : null}
                      </div>
