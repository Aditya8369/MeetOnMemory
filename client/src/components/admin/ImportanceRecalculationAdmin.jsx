...
export default function ImportanceRecalculationAdmin() {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'Failed to load importance recalculation status.')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const handleConfirmRecalculate = async () => {
    ...
    try {
      ...
      toast.success(res.data?.message || 'Importance recalculation job triggered successfully!')
      await fetchStatus()
    } catch (err) {
      console.error('Error triggering importance recalculation:', err)
      toast.error(
        err?.response?.data?.message || 'Failed to trigger importance score recalculation.',
      )
    } finally {
    ...
    return (
      <div className="rounded-2xl border border-rose-200 dark:border-rose-900/40 bg-rose-50/50 dark:bg-rose-950/20 p-8 text-center space-y-4">
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400 max-w-md mx-auto">
          Only Organization Admins or Owners have permission to view status or trigger importance
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 max-w-xl">
            Recalculate dynamic importance ratings for all decisions and action items in your
            organization based on access frequency, age decay, and user feedback ratings.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0 flex-wrap">
          ...
          >
            ...
            <span>{isJobRunning ? 'Job Running...' : 'Recalculate Importance'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
          ...
          <p className="text-[11px] text-slate-400 mt-1">Indexed organization decisions</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
          ...
          <p className="text-[11px] text-slate-400 mt-1">Active & closed task items</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
          ...
          <p className="text-[11px] text-slate-400 mt-2">
            {data?.redisActive ? 'BullMQ Background Queue' : 'Synchronous Execution'}
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-4">
        ...
        {activeJob ? (
          ...
          <div className="rounded-xl border border-rose-200 dark:border-rose-900 bg-rose-50 dark:bg-rose-950/30 p-4 text-sm space-y-1">
            <p className="font-bold text-rose-800 dark:text-rose-300">Last Job Failure:</p>
            <p className="text-xs text-rose-700 dark:text-rose-400 font-mono">{lastRun.error}</p>
          </div>
        ) : (
          <div className="rounded-xl border border-dashed border-slate-200 dark:border-slate-700 p-6 text-center text-xs text-slate-500">
            No recalculation job is currently queued or executing. Click "Recalculate Importance"
            above to initiate a fresh background sweep.
          </div>
        )}
      </div>
