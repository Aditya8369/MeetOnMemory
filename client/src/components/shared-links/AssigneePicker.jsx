...
const AssigneeAvatar = ({ user, size = 'md' }) => {
  ...
  return (
    <div className={`${sizeClasses[size]} rounded-full ${getColor(user._id || user.id)} flex items-center justify-center text-white font-semibold shadow-sm`}>
      {getInitials(user.name || user.email)}
    </div>
  );
};

const AssigneeChip = ({ user, onRemove, size = 'sm', className = '' }) => {
  if (!user) return null;

  return (
    <div className={`inline-flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 rounded-full pl-1 pr-2 py-0.5 border border-slate-200 dark:border-slate-700 ${className}`}>
...
const AssigneePicker = ({
    ...
    try {
      ...
      if (onSearch) {
        ...
        results = users.filter(user => {
          ...
          if (selectedUsers.some(s => (s._id || s.id) === (user._id || user.id))) {
            return false;
          }

          // Exclude explicitly excluded users
          if (excludedUsers.some(e => (e._id || e.id) === (user._id || user.id))) {
            return false;
          }
          ...
          const deptMatch = user.department?.toLowerCase().includes(searchLower) || false;
  ...
  }, [users, selectedUsers, excludedUsers, organizationId, validateRole, validatePermission, onSearch, onError]);
  ...
  const validateUser = useCallback((user) => {
    ...
    if (multiple && selectedUsers.length >= maxAssignees) {
      return { valid: false, error: `Maximum ${maxAssignees} assignees allowed` };
    }

    // Check outside organization
    if (validateOutsideOrg && organizationId && user.organizationId !== organizationId) {
    ...
    if (validatePermission && !user.permissions?.includes(validatePermission)) {
      return { valid: false, error: `User lacks required permission: ${validatePermission}` };
    }
    ...
    return { valid: true };
  }, [selectedUsers, multiple, maxAssignees, validateOutsideOrg, organizationId, validateRole, validatePermission, excludedUsers]);
  ...
  const handleSelectUser = useCallback((user) => {
    ...
    setRecentlySelected(prev => {
      const filtered = prev.filter(u => (u._id || u.id) !== (user._id || user.id));
      return [user, ...filtered].slice(0, 5);
    });

    toast.success(`Assigned to ${user.name || user.email}`);
  }, [selectedUsers, multiple, validateOnSelect, validateUser, onChange, onSelect, onError, disabled, readOnly]);

  // Handle user removal
  const handleRemoveUser = useCallback((user) => {
    if (disabled || readOnly) return;

    const newSelected = selectedUsers.filter(u => (u._id || u.id) !== (user._id || user.id));
    setSelectedUsers(newSelected);
  ...
  }, [isOpen, filteredUsers, focusedIndex, handleSelectUser, performSearch, minSearchLength, searchQuery]);
  ...
  const defaultRenderAssignee = useCallback((user) => {
    return (
      <AssigneeChip
        user={user}
  ...
  const defaultRenderOption = useCallback((user, isFocused, isSelected) => {
    ...
    return (
      <div
        ...
        className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors ${
          isFocused ? 'bg-blue-50 dark:bg-blue-900/30' : 'hover:bg-slate-50 dark:hover:bg-slate-800'
        ...
        {showAvatar && (
          <div className="relative flex-shrink-0">
            ...
            {showPresence && user.status && (
              <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white dark:border-slate-900 ${getStatusColor(user.status)}`} />
            )}
          </div>
        )}
  ...
  }, [showAvatar, showRole, showEmail, showDepartment, showAvailability, showPresence, validateOnSelect, handleSelectUser, filteredUsers, optionClassName]);
  ...
  const renderLoadingState = () => {
    ...
    return (
      <div className="p-4 text-center">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">Loading users...</p>
      </div>
    );
  };
    ...
    >
        ...
        <div className="flex flex-wrap gap-1.5 mb-2">
          {selectedUsers.map((user, index) => (
            <React.Fragment key={user._id || user.id || index}>
              {renderAssignee ? renderAssignee(user, handleRemoveUser) : defaultRenderAssignee(user)}
            </React.Fragment>
          ))}
      ...
      {!readOnly && showSearch && (
        <div className="relative">
          <div className="relative">
            ...
            <input
              ...
              className={`w-full pl-9 pr-10 py-2.5 text-sm rounded-xl border ${
                error ? 'border-red-500 focus:ring-red-500' : 'border-slate-300 dark:border-slate-600 focus:ring-blue-500'
          ...
          {error && (
            <p id="assignee-error" className="mt-1.5 text-xs text-red-500 flex items-center gap-1">
      ...
      {readOnly && selectedUsers.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {selectedUsers.map((user, index) => (
            <div key={user._id || user.id || index} className="flex items-center gap-2">
