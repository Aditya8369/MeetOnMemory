...
export default function GraphSnapshots({ userRole = 'viewer' }) {
  ...
  const executeRestore = async () => {
    ...
    try {
      const res = await fetch(`/api/graph/snapshots/${snapshotToRestore.id}/restore`, {
        method: 'POST',
      });
      if (res.ok) {
        alert(`Successfully reverted knowledge graph tracking back to: ${snapshotToRestore.label}`);
  ...
  return (
    <div className="graph-snapshots-container p-6 bg-slate-900 text-white">
      <h1 className="text-2xl font-bold mb-6">🌐 Knowledge Graph Time-Travel snapshots manager</h1>

      {/* Admin Operations Section: Capture Tool */}
      {isAdmin ? (
        <form onSubmit={handleCreateSnapshot} className="mb-8 p-4 bg-slate-800 rounded-lg border border-slate-700">
          <h2 className="text-lg font-semibold mb-3">📸 Capture Manual Snapshot</h2>
          <div className="flex flex-col gap-3">
            ...
            >
              {isCapturing ? 'Capturing Topology Matrix...' : 'Capture Snapshot Now'}
            </button>
          </div>
        </form>
      ) : (
        <div className="mb-6 p-3 bg-amber-950/40 border border-amber-800 text-amber-300 rounded text-sm">
          ℹ️ Read-Only Mode: Creating or restoring graph states requires administrative access credentials.
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="p-4 bg-slate-800 rounded-lg border border-slate-700">
          <h3 className="text-sm font-medium mb-3 text-slate-400">Side-by-Side Architectural Summary</h3>
        ...
        {diffSummary && (
          <div className="p-4 bg-slate-800 rounded-lg border border-blue-900 text-sm">
            <h4 className="font-semibold text-blue-400 mb-2">⚖️ Meaningful Topology Diffs Summary</h4>
            <ul className="space-y-1 text-slate-300 text-xs">
              <li>• Node Variation: <span className="text-white font-bold">{diffSummary.addedNodes}</span> entities</li>
              <li>• Link Edge Structural Drift: <span className="text-white font-bold">{diffSummary.modifiedEdges}</span> linkages</li>
              <li>• Historical Age Separation: <span className="text-white font-bold">{(diffSummary.timestampDelta / 3600000).toFixed(2)}</span> hours</li>
            </ul>
          </div>
        )}
      </div>
        ...
        <table className="w-full text-left text-xs border-collapse">
          ...
          <tbody>
            {snapshots.map((snapshot) => (
              <tr key={snapshot.id} className="border-b border-slate-700/60 hover:bg-slate-700/30">
                <td className="p-3 text-slate-400">{new Date(snapshot.createdAt).toLocaleString()}</td>
                <td className="p-3 font-medium text-slate-200">
                  {snapshot.label}
                  {snapshot.notes && <p className="text-[11px] text-slate-400 font-normal mt-0.5">{snapshot.notes}</p>}
                </td>
                <td className="p-3 text-slate-300 font-mono">Nodes: {snapshot.nodeCount} | Edges: {snapshot.edgeCount}</td>
                {isAdmin && (
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-slate-800 border border-red-900 p-6 rounded-xl max-w-md w-full shadow-2xl">
            <h3 className="text-lg font-bold text-red-500 mb-2">🚨 Critical Impact Impact Warning</h3>
            <p className="text-slate-300 text-xs mb-4 leading-relaxed">
              You are about to roll back the live database state configuration to the snapshot: 
              <span className="text-white font-bold block mt-1">"{snapshotToRestore?.label}"</span>
              This action terminates active workspace configurations, rewires relational linkages, and overrides any unpersisted entries since this marker was logged.
            </p>
            <div className="flex justify-end gap-3 text-xs font-semibold">
              <button
                onClick={() => { setShowConfirmModal(false); setSnapshotToRestore(null); }}
